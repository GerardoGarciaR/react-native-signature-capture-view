{/* I propose that changes in order to make it work CrossPlatform (web, android and iOS) 
I mean modifying the code as this it works on iOS and android. 
Also It sees on Web. That I'm trying to do is, cuz WebView doesnt not work on Web Platform
I'm using Platform.OS in order to detect when its loaing from web, then Instead of using the
Webview, use an iFrame, and It shows the Canva with the drawing signature successfully!.

Now I'm stucked in how to inject Javascript code to the iFrame, in order to make it work
the Clear and Save Buttons on Web.
...
const sourceHtmlWeb2 = `<h1>Holo</h1>`
...
export class SignatureView extends React.PureComponent {
    ...
    clearSignature = () => {
        ...
        switch (Platform.OS) {
            case 'web':
                selector_canva = 'CLEAR_SIGNATURE';
                break;
    ...
    isEmpty = () => {
        switch (Platform.OS) {
            case 'web':
                selector_canva = 'IS_SIGNATURE_EMPTY';
                break;
    ...
    render() {
        return (
            (Platform.OS === 'web'?
                <iframe 
                    srcDoc={sourceHtmlWeb} 
                    height={'100%'} 
                    width={'100%'} 
                    ref={(ref) => (this.iframe.current = ref)}
                />:
                <View
